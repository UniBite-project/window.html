...
window.renderChefRequests = function() {
    ...
    // Συνάρτηση για φόρτωση των στοιχείων χρήστη στη σελίδα λογαριασμού
function loadUserAccountData() {
    // Προσπάθησε να διαβάσεις τα αποθηκευμένα στοιχεία
    let userData = localStorage.getItem('unibite_user_data');
    let username = localStorage.getItem('unibite_user') || loggedUser;
    let role = localStorage.getItem('unibite_role') || userRole;
    
    // Αν δεν υπάρχουν αποθηκευμένα στοιχεία, δημιούργησε default
    if (!userData) {
        userData = {
            username: username,
            fullName: username === "xristos" ? "Χρήστος Κολιάς" : 
                      username === "maria" ? "Μαρία Παπαδοπούλου" :
                      username === "giorgos" ? "Γιώργος Αντωνίου" : username.charAt(0).toUpperCase() + username.slice(1),
            email: username === "xristos" ? "up1112136@upnet.gr" : username + "@upnet.gr",
            studentId: username === "xristos" ? "1112136" : "ΑΜ-" + username,
            department: "Μηχανικών Ηλεκτρονικών Υπολογιστών & Πληροφορικής",
            registrationDate: "05/06/2026",
            points: 5,
            offers: 12,
            receipts: 8
        };
        localStorage.setItem('unibite_user_data', JSON.stringify(userData));
    } else {
        userData = JSON.parse(userData);
    }
    
    // Ενημέρωση στοιχείων στη σελίδα
    setTimeout(function() {
        ...
        const departmentField = document.querySelector('.academic-table tr:nth-child(4) .academic-value');
        const dateField = document.querySelector('.academic-table tr:nth-child(5) .academic-value');
        
        // Ενημέρωση στατιστικών
        const offersField = document.querySelector('.stat-box:nth-child(2) .stat-number');
        const receiptsField = document.querySelector('.stat-box:nth-child(3) .stat-number');
        
        if (avatarLetter) avatarLetter.innerText = (userData.fullName || userData.username).charAt(0).toUpperCase();
        if (profileNameDisplay) profileNameDisplay.innerText = userData.fullName || userData.username;
        if (roleSubField) roleSubField.innerText = role === "admin" ? "Διαχειριστής UniBite" : "Φοιτητής UniBite";
        if (usernameField) usernameField.innerText = userData.username;
        if (emailField) emailField.innerText = userData.email;
        if (amField) amField.innerText = userData.studentId;
        if (pointsField) pointsField.innerText = userData.points;
        if (offersField) offersField.innerText = userData.offers;
        if (receiptsField) receiptsField.innerText = userData.receipts;
        if (departmentField) departmentField.innerText = userData.department;
        if (dateField) dateField.innerText = userData.registrationDate;
        
        // Ενημέρωση και του welcome message
        const welcomeMsg = document.getElementById('welcomeMsg');
        if (welcomeMsg) {
            const t = translations[currentLang];
            const firstName = (userData.fullName || userData.username).split(' ')[0];
            welcomeMsg.innerHTML = `${t.welcome_prefix} ${firstName}!`;
        }
    }, 100);
}
    ...
    window.loadPage = function(pageId) {
        ...
        switch(pageId) {
            ...
            case 'orders':
                ...
                break;
                // Στο loadPage function, πρόσθεσε νέο case:
            case 'logout':
            case 'delete':
                // Αποσύνδεση χρήστη
                logoutUser();
        ...
        if (pageId === 'home') {
            ...
            if (pageId === 'account') {
                ...
                // Φόρτωσε τα στοιχεία του χρήστη
                setTimeout(function() {
                    loadUserAccountData();
                }, 50);
            }
    ...
    // Υποθέτουμε ότι οι αγγελίες σου βρίσκονται σε έναν πίνακα listings
    const listings = [
    ...
    if (selectedAd) {
        ...
        showSuccessModal(selectedAd.title);
        
        // Αν ο χρήστης είναι ήδη στη σελίδα των παραγγελιών, ανανέωσε την οθόνη
        renderMyOrders();
    } else {
        showErrorModal("Η αγγελία δεν βρέθηκε.");
    }
};

// Συνάρτηση για εμφάνιση επιτυχίας
function showSuccessModal(mealTitle) {
    ...
    modal.innerHTML = `
        <div class="success-modal">
            ...
            <h3>Η παραγγελία στάλθηκε!</h3>
            <p>Η παραγγελία σας για <strong>"${mealTitle}"</strong> στάλθηκε με επιτυχία στον μάγειρα.<br><br>Μπορείτε να την παρακολουθήσετε στην ενότητα <strong>Οι Παραγγελίες μου</strong>.</p>
            <button class="success-modal-btn" onclick="this.closest('.success-modal-overlay').remove()">Τέλεια!</button>
        </div>
    `;
    ...
    // Κλείσιμο κάνοντας κλικ στο overlay (όχι στο modal)
...
function logoutUser() {
    // Επιβεβαίωση αποσύνδεσης (προαιρετικά)
    const confirm = window.confirm('Είσαι σίγουρος ότι θέλεις να αποσυνδεθείς;');
    if (!confirm) return;
    ...
    localStorage.removeItem('unibite_user_data');
    localStorage.removeItem('unibite_points');
    localStorage.removeItem('unibite_transactions');
    localStorage.removeItem('unibite_lang');
    
    // Καθαρισμός sessionStorage
    sessionStorage.clear();
    
    // Προαιρετικά: Αποστολή request στο backend για καταστροφή PHP session
    fetch('logout.php', {
        method: 'POST',
        headers: {
    ...
    // Ανακατεύθυνση στο login page
    window.location.href = 'login.html';
}

// Συνάρτηση για εμφάνιση σφάλματος (προαιρετικά)
function showErrorModal(message) {
    ...
    modal.innerHTML = `
        <div class="success-modal">
            ...
            <p>${message}</p>
            <button class="success-modal-btn" onclick="this.closest('.success-modal-overlay').remove()">Προσπάθησε ξανά</button>
        </div>
    `;
    ...
    // Αντικατέστησε την υπάρχουσα συνάρτηση confirmDeleteAccount (γραμμές ~1080-1090)
// με αυτήν:

window.confirmDeleteAccount = async function() {
    // Πρώτο modal επιβεβαίωσης
    const confirmed = confirm(
        '⚠️ ΠΡΟΣΟΧΗ! ⚠️\n\n' +
        'Η διαγραφή του λογαριασμού σου είναι ΜΟΝΙΜΗ και:\n' +
        '• Θα χάσεις όλους τους πόντους σου\n' +
        '• Οι αγγελίες σου θα διαγραφούν\n' +
        '• Τα αιτήματά σου θα ακυρωθούν\n' +
    ...
    // Δεύτερη επιβεβαίωση με πληκτρολόγηση λέξης
    const code = prompt(
        'Για επιβεβαίωση, πληκτρολόγησε τη λέξη "ΔΙΑΓΡΑΦΗ" στο πεδίο παρακάτω:'
    );
    ...
    const deleteBtn = document.querySelector('.academic-btn-delete');
    if (deleteBtn) {
        deleteBtn.textContent = 'Διαγράφεται...';
    ...
    try {
        ...
        if (result.success) {
            ...
            alert('Ο λογαριασμός σου διαγράφηκε επιτυχώς.');
            
            // Ανακατεύθυνση στην index.html
            window.location.href = 'index.html';
        } else {
            alert('Σφάλμα: ' + (result.message || 'Δεν ήταν δυνατή η διαγραφή.'));
            if (deleteBtn) {
                deleteBtn.textContent = 'Διαγραφή λογαριασμού';
                deleteBtn.disabled = false;
            }
        }
    } catch (error) {
        console.error('Delete account error:', error);
        alert('Σφάλμα επικοινωνίας με τον διακομιστή. Δοκιμάστε ξανά.');
        if (deleteBtn) {
            deleteBtn.textContent = 'Διαγραφή λογαριασμού';
            deleteBtn.disabled = false;
        }
